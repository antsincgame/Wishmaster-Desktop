      ...
      <header className="px-6 py-4 border-b border-cyber-border bg-cyber-surface flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500">
            {currentModel
              ? `Модель: ${currentModel.name}${currentModel.isLoaded ? ' (в памяти)' : ' (выбрана, загрузится при первом сообщении)'}`
              : 'Выберите модель в разделе «Модели»'}
          </p>
        </div>
            ...
            <p className="text-gray-500 max-w-md">
              {currentModel
                ? currentModel.isLoaded
                  ...
                  : 'Напишите сообщение — модель загрузится автоматически при отправке'
                : 'Выберите модель в разделе «Модели» (кнопка «Выбрать» или «Загрузить»)'}
            </p>
          </div>
        )}
